...
class ArticleCategory(models.Model):
    ...
    def __str__(self):
        return self.title #admin站点显示，调试查看对象方便
...
class Article(models.Model):
    ...
    文章
    """
    # 定义文章作者。 author 通过 models.ForeignKey 外键 与内建的 User 模型关联在一起
    # 参数 on_delete 用于指定数据删除的方式，避免两个关联表的数据不一致。(当user表中的用户被删除之后，文章也同步被删除)
    ...
    # 内部类 class Meta 用于给 model 定义元数据（修改表名以及admin展示的配置信息等）
    class Meta:
